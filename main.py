...
def main():
    ...
    what_you_want_from_model = """
    Given the information {information} about a person I want you to create:
    1. Summarize the person's background  
    2. Find 2 interesting facts online  
    3. Identify their role (Founder, CEO, etc.)  
    """

    summary_prompt_template = PromptTemplate(
        input_variables=["information"],
        template=what_you_want_from_model,
    )

    llm = ChatOpenAI(model="gpt-5", temperature=0)

    # Optional alternative: use a local Ollama model instead of OpenAI (commented out)
    # Run as : ollama run gemma3:270m
